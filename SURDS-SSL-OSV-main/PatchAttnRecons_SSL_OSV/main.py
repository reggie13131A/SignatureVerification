...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_model', type=str, default=None)  # path to load model
    # 一张patch下的尺寸大小
    ...
    print(f"Total trainable parameters: {sum(p.numel() for p in ssl_model.parameters() if p.requires_grad)}")
    ...
    while epoch != args.max_epochs:
        ...
        if (epoch+1) % 5 == 0:
            torch.save({'model': ssl_model.state_dict(), 'epochs': epoch+1},
                        f'{args.saved_models}/Chisig_{os.path.basename(args.dataset)}_R={args.round}_SSL.pt')
            torch.save(ssl_model.encoder.state_dict(), f'{args.saved_models}/ChiSig_{os.path.basename(args.dataset)}_R={args.round}_SSL_InceptionResNet.pth')
